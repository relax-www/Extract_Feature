...
if __name__ == "__main__":
    ...
    for sttt in range(100,7100,200):
        ...
        for i in range(0,lag,lag_jiange):
            ...
            if i==0:
                dynamic_origin=T_test[0:window_length,DLV_index-1]
                W_origin=R[:,0]
            else:
                dynamic_origin=np.c_[dynamic_origin,T_test[0:window_length,DLV_index-1]]
                W_origin = np.c_[W_origin,R[:, 0]]
        A=matrix.up_cling_matrix(50,1,-1)+matrix.up_cling_matrix(50,0,1)
        W_new=W_origin@A.T
        plt.plot(W_new[:,:-1].T)
        plt.show()

        # delta_dynamic=np.zeros([window_length,lag-1])
        # qujunzhi=np.zeros([window_length,lag-1])
        for i in range(0, dynamic_origin.shape[1]-1):
            delta_dynamic_value=dynamic_origin[:,i+1]-dynamic_origin[:,i]
            delta_dynamic_value=delta_dynamic_value.reshape([window_length,1])
            qujunzhi_value=dynamic_origin[:,i+1]-dynamic_origin[:,0]
            qujunzhi_value=qujunzhi_value.reshape([window_length, 1])
            if i==0:
                delta_dynamic=delta_dynamic_value
                qujunzhi=qujunzhi_value
            else:
                delta_dynamic=np.append(delta_dynamic,delta_dynamic_value,axis=1)
                qujunzhi=np.append(qujunzhi,qujunzhi_value,axis=1)
        # delta_dynamic=delta_dynamic[:,:lag-1]


        # print(phi_v_lim, phi_s_lim)
        output = x_train[range(window_last,window_last+lag,lag_jiange), 0]
        # output = np.array(filter.MedianAverage(x_train[window_last:window_last + lag, 0], 10))
        result = filter.Butterworth_filter(x_train[window_last:window_last + lag, 0], 3)
        # # 平均中值滤波
        # # result = filter.MedianAverage(x_train[window_last:window_last+lag,0], 10)
        # # 滑动滤波
        # result = filter.SlidingAverage(x_train[window_last:window_last + lag, 0], 10)
        # # 巴特沃夫滤波器，双向
        # result=filter.Butterworth_filter(x_train[window_last:window_last + lag, 0],3)
        # result=result[range(0,result.shape[0],10)]
        output=result[range(0,lag,lag_jiange)]

        # delta_output=np.zeros([lag])
        for i in range(0,output.shape[0]-1):
            if i==0:
                delta_output=output[i+1]-output[i]
            else:
                delta_output=np.append(delta_output,output[i+1]-output[i])
        # delta_output = delta_output[:lag - 1]

        sigma=np.std(delta_dynamic,axis=0)
        mean=np.mean(delta_dynamic,axis=0)

        fig1=plt.figure(figsize=(10, 6), dpi=200)
        plt.subplot(3,1,1)
        plt.plot(range(0,window_length,5),delta_dynamic[range(0,window_length,5),0:delta_dynamic.shape[0]],label=list(range(0,delta_dynamic.shape[1])))
        plt.ylabel("一阶差分\n{start}--{last}".format(start=window_start,last=window_last),rotation=0)
        plt.legend(loc='right')
        plt.grid()
        plt.subplot(3, 1, 2)
        # plt.plot(output,label='output')
        plt.plot(result,label='output')#未进行采样
        plt.plot(x_train[window_last:window_last + lag, 0], label='origin')
        plt.ylabel("output间隔为{jiange}\nlag={lag}".format(jiange=lag_jiange,lag=lag),rotation=0)
        # plt.plot(sigma*1000,label='sigma')
        # plt.plot(mean , label='mean')
        plt.legend(loc='upper right')
        plt.grid()
        plt.subplot(3, 1, 3)
        plt.plot(range(0, window_length, 5), qujunzhi[range(0, window_length, 5), 0:qujunzhi.shape[1]],
                 label=list(range(0, qujunzhi.shape[1])))
        plt.ylabel("减去第一个值的差\n".format(),rotation=0)
        plt.legend(loc='right')
        plt.grid()

        plt.savefig("./figure2/"+"G阶跃向上20h_lag=100_d=5_2/"+str(sttt)+".jpg")  ## 保存图片

        # np.savetxt("./delta_dynamic{i}.txt".format(i=sttt),delta_dynamic, fmt='%f', delimiter=',')
        # b = np.loadtxt("./delta_dynamic{i}.txt".format(i=sttt), delimiter=',')
        result = filter.MedianAverage(x_train[window_last:window_last + lag, 0], 10)
        result = x_train[range(window_last,window_last + lag,5), 0]
        xian = cluster_exp.GaussianMixture_extract(delta_dynamic, jiangge=5, cluster_num=4, pic_flag=1)
        xian_integral = xian @ matrix.up_triangle_matrix(xian.shape[0], xian.shape[0])
        corr_exp=corr.calc_corr2(xian,result)
        # delta_dynamic_mean=delta_dynamic-np.mean(delta_dynamic,axis=1).reshape(delta_dynamic.shape[0],1)@np.ones([1,delta_dynamic.shape[1]])
        # ss=matrix.up_cling_matrix(delta_dynamic.shape[1],0,1)+matrix.up_cling_matrix(delta_dynamic.shape[1],1,0.6)+\
        #    matrix.up_cling_matrix(delta_dynamic.shape[1],2,0.4)+matrix.up_cling_matrix(delta_dynamic.shape[1],3,0.2)\
        #    +matrix.up_cling_matrix(delta_dynamic.shape[1],4,0.1)

        # # delta_dynamic=np.abs(delta_dynamic)
        ...
        plt.grid()
        plt.subplot(2, 1, 2)#&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
        # plt.plot(delta_dynamic.T[:, range(0, window_length-180 , 20)],label="origin")
        plt.plot(xian,label="origin")
        plt.plot(xian_integral,label="积分")
        # plt.plot(delta_dynamic_mean_sum.T[:, range(0, window_length - 180, 20)],label="积分")
        # plt.plot(qujunzhi.T[:, range(0, window_length - 180, 20)],label="123")
        plt.legend(loc="right")
        plt.grid()
        plt.savefig("./figure2/" + "G阶跃向上20h_lag=100_d=5_2/" + str(sttt) +"00"+ ".jpg")  ## 保存图片
        plt.show()
        plt.close(fig1)
        plt.close(fig2)
        a=1
