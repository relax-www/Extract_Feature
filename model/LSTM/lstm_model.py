...
if __name__=="__main__":
    ...
    data1 = loadmat(r"C:\Users\zs\matlabProject\temexd_mod\data\xmv.mat")["xmv"]
    data2 = loadmat(r"C:\Users\zs\matlabProject\temexd_mod\data\xmeas.mat")["simout"]

    data=np.append(data1,data2[:,20].reshape([data1.shape[0],1]),axis=1)
    ...
    for i in range(0,5500,100):
        trainX1, trainY1 = createXY(df_for_training_scaled[i:100+i,:], time_step)
        testX1, testY1 = createXY(df_for_testing_scaled[i:100+i,:], time_step)
        if i==0:
            trainX, trainY = trainX1, trainY1
            testX, testY = testX1, testY1
        else:
            trainX=np.append(trainX,trainX1,axis=0)
            trainY = np.append(trainY, trainY1, axis=0)
            if testX1.size!=0:
                testX=np.append(testX,testX1, axis=0)
                testY=np.append(testY,testY1, axis=0)
    ...
    model=Model_lstm(n_feature)
    model=train_model(model, X_train, Y_train,train_history_path)
    model.save(r"model.h5")
    model=load_model(r"model.h5")
    # 删除当前已存在的模型
    # del model
    pred = np.squeeze(model.predict(trainX))
    plt.figure()
    plt.subplot(2,1,1)
    plt.plot(pred,label="pred")
    plt.plot(trainY,label="Y_test")
    plt.legend(loc="right")
    plt.subplot(2, 1,2)
    plt.plot(pred-trainY)
    plt.show()
    pred = pred.reshape(pred.shape[0], 1)
    RMSE=caculate_RMSE(Y_test,pred)
    print(RMSE)